...
import { PRODUCT_CSV_FILE, TOKEN } from '../utils/constants'
import productFactory from '../utils/factories/ProductFactory'
import { checkErrorFormat } from '../utils/formats/ErrorFormat'
import { checkInsertManyProductsFormat, checkPaginateProductsFormat, checkProductFormat, productResponse } from '../utils/formats/ProductFormat'
...
describe('Product', () => {
  ...
  describe('get product', () => {
    describe('basic positive tests', () => {
      ...
      describe('validate payload', () => {
        ...
        it('should return a product object with valid values in get one product', async () => {
          const temp = productFactory()
          const { body } = await requestApp.post('/api/v1/product').set(TOKEN).send(temp)
  ...
  describe('create product with csv', () => {
    ...
    //     //     ...temp,
    //     //     bar_codes: '1234567890123',
    //     //   }

    //     //   await requestApp.post('/api/v1/product').set(TOKEN).send(duplicatedProduct)

    //     //   const response = await requestApp.post('/api/v1/product').set(TOKEN).send(duplicatedProduct)
